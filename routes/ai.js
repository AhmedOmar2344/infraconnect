...
// Guards against the AI occasionally returning something that isn't a
// real translation at all — a bare "0", "null", or a string too short to
// plausibly be an actual product name — which `value || ''` doesn't catch
// (any non-empty string, "0" included, is truthy in JS). Treating these
// as invalid (rather than storing them) is what actually fixes a product
// showing the literal text "0" as its name in Arabic mode instead of a
// real translation or a graceful English fallback.
function isValidTranslatedText(value, minLength = 2) {
  if (typeof value !== 'string') return false;
  const trimmed = value.trim();
  if (trimmed.length < minLength) return false;
  if (/^(0|null|undefined|n\/?a|none)$/i.test(trimmed)) return false;
  if (/^\d+$/.test(trimmed)) return false; // purely numeric — not a real name/description
  return true;
}
  ...
  try {
    ...
    for (const p of products) {
      try {
        ...
        if (!isValidTranslatedText(translated.name_ar)) {
          results.push({ id: p.id, name: p.name, status: 'failed: AI returned an invalid name_ar (e.g. "0" or empty) — not stored' });
          continue;
        }
...
/**
 * GET /api/ai/auto-translate-cron
 *
 * The actual "no need to manually translate anything" system — triggered
 * automatically on a schedule by Vercel Cron (see vercel.json), not by an
 * admin clicking a button. Each invocation processes one batch of
 * whatever's still missing an Arabic translation; since it repeats on a
 * schedule, the whole catalog gets covered over time with zero manual
 * work, rather than requiring "Translate Missing Arabic" to be clicked
 * repeatedly until finished.
 *
 * Uses Groq by default — genuinely free, no balance to run out, which
 * matters for something running unattended and repeatedly rather than a
 * single manual click an admin is watching.
 *
 * SECURITY: same pattern as monthly-report-cron in routes/analytics.js —
 * Vercel Cron calls this via a plain GET with an
 * `Authorization: Bearer <CRON_SECRET>` header (can't carry a normal
 * admin login token), so this checks for that specifically, with a
 * logged-in-admin fallback for manual testing.
 */
router.get('/auto-translate-cron', async (req, res, next) => {
  try {
    ...
    if (!isCron) {
      ...
      try {
        ...
        if (decoded.role !== 'admin' && decoded.role !== 'superadmin' && decoded.role !== 'poweruser') {
          return res.status(403).json({ error: 'Not authorized.' });
        }
      } catch {
        return res.status(401).json({ error: 'Not authorized — this endpoint requires the cron secret or an admin login.' });
      }
    }
    ...
    try { keys = await resolveProviderKeys(provider); }
      ...
      console.error('[auto-translate-cron] No provider key configured:', e.message);
      return res.status(200).json({ processed: 0, remaining: null, note: 'No Groq key configured — nothing to do.' });
    }

    // A moderate batch per run — this fires repeatedly on its own
    // schedule, so it doesn't need to rush through everything in one
    // invocation the way a single manual click does.
    const batchSize = 30;
    const products = await db.prepare(
      `SELECT id, name, description, specs, badge FROM products
       WHERE active=1 AND (name_ar IS NULL OR name_ar='') ORDER BY id LIMIT ?`
    ).all(batchSize);
    ...
    for (const p of products) {
      try {
        const specsArr = p.specs ? p.specs.split('|') : [];
        const result = await translateToArabic({ ...p, specs: specsArr }, provider, keys);
        if (!result || !isValidTranslatedText(result.name_ar)) { failed++; continue; }
        const specsArStr = Array.isArray(result.specs_ar) ? result.specs_ar.join('|') : (result.specs_ar || '');
        await db.prepare(
          `UPDATE products SET name_ar=?, description_ar=?, specs_ar=?, badge_ar=?, updated_at=datetime('now') WHERE id=?`
        ).run(result.name_ar, result.description_ar || '', specsArStr, result.badge_ar || '', p.id);
        translated++;
      } catch (e) {
        console.error(`[auto-translate-cron] Failed on product ${p.id}:`, e.message);
        failed++;
      }
    }

    const remainingRow = await db.prepare(`SELECT COUNT(*) as c FROM products WHERE active=1 AND (name_ar IS NULL OR name_ar='')`).get();
    const remaining = Number(remainingRow.c);
    console.log(`[auto-translate-cron] Translated ${translated}, failed ${failed}, ${remaining} still remaining.`);
    res.json({ translated, failed, remaining });
  } catch (err) { next(err); }
});
