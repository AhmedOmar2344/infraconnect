...
router.get('/', async (req, res, next) => {
  try {
    const db = getDb();
    const { category, featured, search, limit=100, page=1 } = req.query;
...
function isModelToken(tok) {
  return /[a-z]/.test(tok) && /[0-9]/.test(tok) && tok.length >= 5 && !GENERIC_SPEC_TERMS.has(tok);
}
...
router.post('/bulk-assign-from-library', auth, requireAdmin, async (req, res, next) => {
  try {
    ...
    let products;
    if (useAllMode) {
      products = await db.prepare(`SELECT id, name FROM products WHERE active=1 AND image IS NULL ORDER BY id LIMIT ?`).all(limit);
    } else {
    ...
    if (!libraryPhotos.length) return res.json({ matched: 0, total: products.length, results: [], remaining: useAllMode ? Number((await db.prepare(`SELECT COUNT(*) as c FROM products WHERE active=1 AND image IS NULL`).get()).c) : 0, error: 'Library is empty — nothing to match against.' });
    ...
    for (const product of products) {
      ...
      for (const lib of libIndexed) {
        ...
        else if (sharedBrand.length && shared.length >= 3) score = shared.length;
        else continue;
    ...
    const remaining = useAllMode
      ? Number((await db.prepare(`SELECT COUNT(*) as c FROM products WHERE active=1 AND image IS NULL`).get()).c)
      : 0;
